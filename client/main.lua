...
function FishGUI(bool)
	...
	text(0.4,0.05,0.35, "Você tem um, você pode fazer isso pressionando [E]")
end
		...
		if not IsPedInAnyVehicle(GetPed(), false) then
			if not IsPedSwimming(GetPed()) then
				if IsEntityInWater(GetPed()) then
					...
					IsFishing = true
					if ShowChatMSG then ESX.ShowNotification("Você lançou sua isca, espere um peixe morder...") end
					RunCodeOnly1Time = true
					BarAnimation = 0

				else
					ESX.ShowNotification('Ação impossível, você deve estar na água')
				end
			else
				ESX.ShowNotification('Ação impossível')
			end
		else
			ESX.ShowNotification('Ação impossível')
		end
	else
		ESX.ShowNotification("Você deve ser um peixeiro para pescar.")
	end
end)
		...
		while CFish do
			...
			if IsControlJustPressed(1, Caught_KEY) then
				if BarAnimation >= SuccessLimit then
					...
					Citizen.Wait(200)
					DeleteEntity(FishRod)
					if ShowChatMSG then ESX.ShowNotification("O peixe escapou!") end
				end
			end
		end
	end
